...
export default function Home() {
    ...
    const generateP = () => {
        ...
        if (a === 0) {
            ...
            setP(
                `$P = ${a !== 1 ? a : ""}x^2 ${c > 0 ? "+" : ""} ${c !== 1 ? c : ""} $`,
            );
            return;
        } else if (c === 0) {
            setP(
                `$P = ${a !== 1 ? a : ""}x^2 ${b > 0 ? "+" : ""} ${b !== 1 ? b : ""}x $`,
            );
            return;
        }
        setP(
            `$P = ${a !== 1 ? a : ""}x^2 ${b > 0 ? "+" : ""}${b !== 1 ? b : ""}x ${c > 0 ? "+" : ""}${c !== 1 ? c : ""} $`,
        );
    };
    ...
    const onSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (
            (predR1 === root1 && predR2 === root2) ||
            (predR1 === root2 && predR2 === root1)
        ) {
    ...
    return (
        <div className="grid items-center justify-items-center min-h-screen place-content-center min-w-screen">
            <main className="flex flex-col align-middle items-center text-center justify-center m-auto min-w-screen">
                <h1 className="sm:text-4xl my-3 font-medium tracking-wider text-2xl">
                ...
                <form
                    className="p-6 bg-black shadow-md rounded-lg"
                    onSubmit={onSubmit}
                >
                    <h3 className="sm:text-xl mb-4 text-lg">
                        Racines réelles:
                    </h3>
